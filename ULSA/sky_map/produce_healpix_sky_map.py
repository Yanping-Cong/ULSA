...
from ULSA.emissivity_fitting.save_fit_params import free_free
import ctypes as ct
import numpy as np
import os
import sys
...
class absorption_JRZ(object):
    ...
    def __init__(self, v, nside, clumping_factor, index_type, distance,emi_form,I_E_form,R0_R1_equal,using_raw_diffuse,test=False, using_default_params=True, params_408 = np.array([71.19, 4.23, 0.03, 0.47, 0.77]),critical_dis=False,output_absorp_free_skymap=False):
        self.v = v
        self.nside = nside
    ...
    def freq_dependence_index_minus_I_E(self,freq):
        if self.using_default_params == True:
            ...
            beta0 = self.using_default_params
    ...
    def pixel_dependence_index_minus_I_E(self):
        if self.using_default_params == True:
            with h5py.File(self.file_dir1+'/index_map.hdf5','r') as f:
                index = f['new_map'][:]
        elif self.using_default_params == False:
            f = reborn_direction()
    ...
    def diffuse_x(self, freq):
        ...
        data_diffuse = hp.read_map(self.file_dir2 + '/sky_map_10-2300MHz/haslam408_dsds_Remazeilles2014.fits')
    ...
    def model_m4(self,l,b,params_408,pix_number,R_2 = 0.1):
        ...
        #f = produce_index(Nside = self.nside, freq = self.v, index_type = self.index_type,I_E_form = self.I_E_form,using_default_params = self.using_default_params)
